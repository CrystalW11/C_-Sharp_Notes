...
3. Search each dog's pet description for a term match
...
3. Add code to gather user input for the `dogCharacteristic` string. Gather the input requires a `while` loop that continues to prompt the user until they submit an input. The loop instructs the user to `Enter one desired dog characteristic to search for`. Entering an empty string repeats the loop. Place the following code following `case "2:` just before the `break;` statement as shown:
...
4. Save your code, then compile by using `dotnet build` in the TERMINAL (you can refer to the steps in the previous exercise) and fix any errors.

5. Run the code by using `dotnet run`.

6. To test the search term input functionality of the app, enter `2`, selecting Menu Option `2`, for searching.

7. Press "**Enter**" without entering data at the "***Enter one desired dog characteristic to search for***" prompt. The program should return to the `Enter one desired dog characteristics to search for` prompt.

8. The program should return to the prompt until characters are entered.

9. At the prompt, test search term entry, with `golden` as your entry. The program should return to the menu without error.

10. At the menu, type "Exit" to exit the program.
...
2. Take a minute to examine the loop added to the end of the `case "2":` code.
...
3. Compile your code by using `dotnet build` and fix any errors.
...
2. Populate the `dogDescription` variable with the values for `animalPhysicalDescription` and `animalPersonalityDescription;`.
...
2. Add the code `noMatchesDog = true`:
...
3. In the brackets of the `if (dogDescription.Contains(dogCharacteristic))` statement, add the following code:

    ```
    noMatchesDog = false;
    ```

4. Review that your code within the brackets of `if (dogDescription.Contains(dogCharacteristic))` was entered properly.
...
5. At the end of `case "2"` code, just before the code `Console.WriteLine("\n\rPress the Enter key to continue");`, which is before the `break;`, add the following code:
...
2. Run your project code in the terminal with `dotnet run`.
...
2. Type `exit`, at the app menu, to end the program and then close the terminal panel.
