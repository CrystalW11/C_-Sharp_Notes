...
3. Take a few minutes to review the initial variable declarations at the top of the **Program.cs** file.
...
4. Scroll down the Program.cs file to examine the `for` loop that contains a `switch` selection construct within its code block.
...
5. Notice that the `for` loop uses the`maxPets` variable to establish an upper bound on the number of iterations following comment # 4.

6. Also notice that the `switch` construct selectively branches the code so that you can define different pet characteristics for the pets in the sample data set.
...
7. Scroll to the bottom of the code file in Visual Studio Code. Examine the code used to display the menu options and capture the input of the user's selection.
...
8. Take a minute to review the two `case` statements.
...
9. Notice the line of code is `readResult = Console.ReadLine();` is followed by a check for a null value.
...
2. Enter: `1`, followed by pressing the "Enter" key to "display all pets."

3. Check that all of the pet information displays.
...
4. Press the Enter key to continue and return to the menu.

5. At the menu prompt, enter `2` followed by pressing the "Enter" key.

    This choice is a place holder for "Display all dogs with a specified characteristic" functionality.

6. Check that the `UNDER CONSTRUCTION` message displays for `Display all dogs with a specified characteristic` selection.
