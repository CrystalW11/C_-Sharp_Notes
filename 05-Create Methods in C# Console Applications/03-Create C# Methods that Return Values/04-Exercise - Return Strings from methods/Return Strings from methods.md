...
3. The method needs to iterate through the given word and update the result. To do this, update the `ReverseWord` method with the following code:
...
2. If necessary, open Visual Studio Code's Integrated Terminal panel.

3. At the Terminal command prompt, enter **dotnet run**. Compare your output with the following:
...
2. Next, you can extract the individual words from the string using `string.Split`. Update the `ReverseSentence` method to the following:
...
3. Update the `ReverseSentence` method to the following:
...
4. You can remove the extra space at the end using `string.Trim`. Update the method to the following code:
...
5. Update the `input` text and the `Console.WriteLine` statement to the following:
