...
3. There are many approaches to solve this problem. Take a moment to consider how you might search for two numbers in an array whose sum is equal to a given value.

In this exercise, the following approach will be used:
    a. Choose one number from the array
    b. Check other numbers one at a time to see if they add up to the target value
    c. Return the result as soon as a match is found

4. To check each number in the array, update the `TwoCoins` method with the following code:
...
5. Next, add logic to check the sum of the two coins for the target value. To do so, update the previous `for` loops with the following code:
...
2. Enter a new blank code line. Then enter the following code:
...
4. At the Terminal command prompt, enter **dotnet run**. Compare your output with the following:
...
2. Update the `TwoCoins` method with the following code:
...
3. Update the `TwoCoins` method with the following code:
...
4. Update the logic in the `TwoCoins` method with the following code:
...
5. Navigate to the `return` statement in the `TwoCoins` method. Modify the `return` statement to match the following code:
...
6. At this point, the `TwoCoins` method should match the following code:
...
2. Navigate to the `Console.WriteLine` call. Update your code to match the following:
...
6. Next, update the value of `target` to a value of `80`:

```
int target = 80;
```

7. Save your work then enter **dotnet run** at the Terminal command prompt

8. To verify that your code is working as expected, compare the output of your application with the following output:
