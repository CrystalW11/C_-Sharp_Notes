...
2. Change `groups` to an optional parameter by updating the method to the following code:
...
4. Update your code with the following nested `for` loop:
...
5. Update your `for` loop with the following code:
...
6. Locate the `AssignGroup()` method call and remove the starting `//` characters.
...
5. Verify that the expected "School A" message is displayed and that there are no error messages.
