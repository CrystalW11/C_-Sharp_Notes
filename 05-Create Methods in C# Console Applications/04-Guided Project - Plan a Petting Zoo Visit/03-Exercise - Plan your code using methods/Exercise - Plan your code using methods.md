...
2. Enter a new blank code line below the `pettingZoo` array initialization.

3. The first task is to randomize the animals. Create a pseudo-code method to randomize the petting zoo animals by entering the following code into the editor:
...
4. Next, create a pseudo-code method to assign the animal groups. Enter the following code into the editor:
...
5. You can easily print the school name using `Console.Write`. Enter the following code into the editor:

```
Console.WriteLine("School A");
```

6. Lastly, you'll need to print the animal groups. Enter the following code into the editor:
...
7. Assign the `group` argument by updating the `AssignGroup` line of code to the following:

```
// var group = AssignGroup();
```

8. Since you have completed pseudo-code for the general tasks, you can take a moment to consider what data type to assign to the `group` variable.
...
9. Assign the `group` data type by updating the `AssignGroup` line of code to the following:
...
3. Verify that the "School A" message is displayed.
