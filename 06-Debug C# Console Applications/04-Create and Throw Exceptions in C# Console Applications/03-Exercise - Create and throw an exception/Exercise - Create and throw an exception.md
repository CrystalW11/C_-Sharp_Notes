...
5. Name the new folder **ThrowExceptions101**, and then select **Select Folder**.
...
7. At the TERMINAL panel command prompt, enter the following command:
...
5. Take a minute to review the code.
    ...
    i. Declares local variables used in calculations.

    ii. Uses a `for` loop to sum the even numbers between `lowerBound` and `upperBound`. The sum is stored in `sum`.

    iii. Counts how many numbers are included in the sum. The count is stored in `count`.

    iv. Stores the average of the summed numbers in a variable named `average`. The value of `average` is returned.
...
2. In the launch.json file, update the console attribute as follows:
...
3. Save your changes to the launch.json file, and then close the file.

4. On the Visual Studio Code **Run** menu, select **Start Debugging**.

5. Switch to the TERMINAL panel.

6. At the "lower bound" prompt, enter **3**

7. At the "upper bound" prompt, enter **11**

8. Notice that the application displays the following message and then pauses:

```
The average of even numbers between 3 and 11 is 7.
```

9. To exit the application, press Enter.
...
2. Consider which exception type to throw.
...
3. At the top of the `AverageOfEvenNumbers` method, to detect the upper bound issue, update your code as follows:
...
4. To create and throw an `ArgumentOutOfRangeException` exception, update the `if` code block as follows:
...
2. To enclose the `AverageOfEvenNumbers` method call and `Console.WriteLine` statement inside a try code block, update your code as follows:
...
3. To create the associated `catch` clause, enter the following code:
...
4. Take a minute to consider how you might handle the exception.
...
5. To enclose the `try` and `catch` blocks inside a `do` loop, update your code as follows:
...
6. To explain the issue to the user and obtain a new `upperBound`, update your `catch` code block as follows:
...
7. To provide the user with an option to exit the loop rather than enter a new upper bound, update your `catch` code block as follows:
...
8. Take a minute to consider the `while` expression required for the `do` loop.
...
9. To instantiate a Boolean variable named `exit`, and use `exit` to set the exit condition of the `do` loop, update your code as follows:
...
10. Save your updated code.

11. On the **Run** menu, select **Start Debugging**.

12. Switch to the TERMINAL panel.

13. At the "lower bound" prompt, enter **3**

14. At the "upper bound" prompt, enter **3**

15. Notice that the following output is displayed in the TERMINAL panel:
...
16. At the prompt for a new upper bound, enter **11**

17. Notice that the following output is displayed in the TERMINAL panel:
...
18. To exit the application, press Enter.
