...
2. In the EXPLORER view, expand the **GuidedProject** and **Starter** folders.
...
4. On the View menu, select **Command Palette**.

5. At the command prompt, enter **.net: g** and then select **.NET: Generate Assets for Build and Debug**.

6. At the **Select the project to launch** prompt, select the **Starter** project.

    The `launch.json` file that's created will include a configuration for the Starter project.

7. Take a few minutes to review the top-level statements for this application:
...
8. Take a moment to review the `LoadTillEachMorning()` method.
...
9. Take a couple minutes to review the `MakeChange()` method.
...
10. Take a moment to review the `LogTillStatus()` method.
...
11. Take a moment to review the `TillAmountSummary()` method.
...
2. Notice that an `IOException` error occurs.
...
3. On the **Debug toolbar**, select **Stop**.

4. Use the EXPLORER view to open the **launch.json** file.

5. In the launch.json file, update the `console` attribute as follows:
...
6. Save your changes to the **launch.json** file.

7. On the Visual Studio Code **Run** menu, select **Start Debugging**.

8. Review the output generated by the application in the TERMINAL panel.

    Switch from the DEBUG CONSOLE panel to the TERMINAL panel to review the output.

9.  Notice that `MakeChange` successfully balances the till when using the `testData` array to simulate transactions.
...
10. To exit out of the application, press Enter.
...
2. Save the **Program.cs** file, and then run the application in the debugger.

3. Review the output in the TERMINAL panel.

4. Notice the discrepancy in the till balance.
...
5. To exit the application, press Enter.
...
2. Set a breakpoint on selected code line.

3. On the Visual Studio Code **Run** menu, select **Start Debugging**.

4. Notice that code execution pauses on the breakpoint.

5. On the **Debug controls** toolbar, select **Step Into**.

6. Review the output in the TERMINAL panel.

7. If the reported and expected till values are equal, select **Continue** on the **Debug controls** toolbar.

8. Repeat the previous step until you see a discrepancy between the reported and expected till values.

9. Once you see a discrepancy, take a minute to examine the details of the transaction.

10. Notice that the reported cash received and change returned are correct, but the till is short by five dollars.

    This shortage suggests that the `cashTill` array is being updated incorrectly, even though the reporting is correct.

11. Stop the debug session and close the TERMINAL panel.

12. Scroll to the bottom of the `MakeChange` method.

    The `while` statements used to "make change" are located at the end of the `MakeChange` method.

13. Review the `while` statement code blocks that are used to make change.
...
15. Update the `while` code block as follows:
...
2. On the **Run** menu, select **Start Debugging**.

3. Review the output in the TERMINAL panel.

4. Verify that the reported till value is equal to the expected till value:
