...
2. Update `MakeChange` method: Create and throw exceptions for "Insufficient till" and "Underpayment" issues.
...
3. Take a minute to consider the purpose of this code.
...
4. To enclose the call to the `MakeChange` method in a `try` statement code block, update your code as follows:
...
5. Add the following `catch` clause after the `try` statement code block:
...
3. Take a minute to consider the issue that this code is addressing.
...
5. Scroll down to locate the following code lines:
...
6. Take a minute to consider the issue that this code is addressing.
...
2. To catch the exception type thrown in the `MakeChange` method, update the code as follows:
...
3. Use the **File** menu to save your updates.
...
2. To convert the `MakeChange` method from type `string` to type void, update the code as follows:

```
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
```

3. Delete the following variable declaration:

```
string transactionMessage = "";
```

4. Scroll to the bottom of the `MakeChange` method.

5. Delete the following code lines:
...
6. Scroll up to the top-level statements and locate the `try` code block.

7. Update the `try` code block as follows:
...
8. Locate and then delete the following code lines:
...
9. Use the **File** menu to save your updates.
...
2. Locate the following code a few lines before the start of the `while` loop.

```
int transactions = 10;
```

3. Update the number of transactions to `40` as follows:

```
int transactions = 40;
```

4. Locate the following code line inside the `while` loop.

```
int itemCost = valueGenerator.Next(2, 20);
```

5. Update the `itemCost` random number generator as follows:
...
6. Use the **File** menu to save your updates.

7. On the **Run** menu, select **Start Debugging**.

8. Review the output in the TERMINAL panel.

9. Verify that the messages associated with the two exception types are displayed:
