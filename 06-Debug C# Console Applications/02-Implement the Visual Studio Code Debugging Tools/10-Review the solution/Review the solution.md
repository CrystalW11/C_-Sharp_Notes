...
3. At the top of the RUN AND DEBUG view, select **Start Debugging**.

4. In the VARIABLES section of the **Run and Debug** view, make note of the value assigned to `x`.

5. On the **Debug control** toolbar, select **Step Into**.

6. Track the value assigned to `x` as you step through each code line.

7. Notice that the value of `x` doesn't change as execution enters and exits the `ChangeValue` method.
