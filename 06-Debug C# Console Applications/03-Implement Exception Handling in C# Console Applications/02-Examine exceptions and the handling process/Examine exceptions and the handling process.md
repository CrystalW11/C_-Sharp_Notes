...
- The `ry-finally` pattern consists of a `try` block followed by a `finally` block. Typically, the statements of a finally block run when control leaves a `try` statement.
